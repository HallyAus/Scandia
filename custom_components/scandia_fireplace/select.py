...
from .const import CONF_CODE_FLAME_SPEED, DEFAULT_FLAME_SPEEDS
...
async def async_setup_entry(
    ...
    if get_code(entry, CONF_CODE_FLAME_SPEED):
...
class ScandiaFlameSpeed(ScandiaEntity, SelectEntity):
    ...
    def __init__(self, coordinator, entry: ScandiaConfigEntry) -> None:
        """Cache the code and available options."""
        super().__init__(coordinator)
        self._code_speed = get_code(entry, CONF_CODE_FLAME_SPEED)
    ...
    def current_option(self) -> str | None:
        ...
        value = self._code_value(self._code_speed)
        return str(value) if value is not None else None

    async def async_select_option(self, option: str) -> None:
        """Change the flame speed."""
        await self.coordinator.async_send([{"code": self._code_speed, "value": option}])
